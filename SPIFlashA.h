...
class SPIFlashA {
public:
  static uint8_t UNIQUEID[12];
  SPIFlashA(uint8_t slaveSelectPin, uint32_t jedecID=0);
  boolean initialize();
  ...
  void blockErase512K(uint32_t address);		// New for SPANSION
...
protected:
  ...
  uint32_t _jedecID;					// Changed form uint16_t to uint32_t for SPANSION
  uint8_t _SPCR;
  uint8_t _SPSR;
